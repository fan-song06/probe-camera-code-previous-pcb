...
import com.lightresearch.probecamera.ausbc.utils.MultiCameraClient.Companion.MAX_NV21_DATA
...
import com.lightresearch.probecamera.ausbc.utils.MultiCameraClient.Companion.CAPTURE_TIMES_OUT_SEC
import java.io.File
...
class CameraUVC(ctx: Context, device: UsbDevice) : MultiCameraClient.ICamera(ctx, device) {
     ...
     * Set Exposure Mode
     */
    fun setExposureMode(exposureMode: Int) {
        mUvcCamera?.exposureMode = exposureMode
    }

    /**
     * Get ExposureMode
     */
    fun getExposureMode() = mUvcCamera?.exposureMode
     ...
     * Get WB
     */
    fun getWhiteBlance() = mUvcCamera?.whiteBlance
