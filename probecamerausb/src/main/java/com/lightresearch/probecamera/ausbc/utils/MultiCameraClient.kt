...
import com.lightresearch.probecamera.ausbc.callback.IDeviceConnectCallBack
import com.lightresearch.probecamera.ausbc.utils.CameraUtils.getParentDirectory
import com.lightresearch.probecamera.ausbc.utils.CameraUtils.isFilterDevice
import com.lightresearch.probecamera.ausbc.utils.CameraUtils.isUsbCamera
...
class MultiCameraClient(ctx: Context, callback: IDeviceConnectCallBack) {
     ...
     */
    fun getDeviceList(list: List<DeviceFilter>? = null): List<UsbDevice>? {
        list?.let {
            addDeviceFilters(it)
        }
    ...
    abstract class ICamera(val ctx: Context, val device: UsbDevice): Handler.Callback,
            ...
            when (msg.what) {
                ...
                MSG_CAPTURE_IMAGE -> {
                    (msg.obj as Pair<*, *>).apply {
                        ...
                        if(path == null) throw IllegalArgumentException("path missing")
                        val dir = getParentDirectory(path)
                            ?: throw java.lang.IllegalArgumentException("path is not to a file")
        ...
        private fun captureVideoStartInternal(path: String?, durationInSec: Long, callBack: ICaptureCallBack) {
            ...
            captureStreamStartInternal()
            Mp4Muxer(mContext, callBack, path, durationInSec, true).apply {
...
//            }
        ...
        private fun getDefaultCameraRequest(): CameraRequest {
            return CameraRequest.Builder()
                .setPreviewWidth(720)
                .setPreviewHeight(1280).setDefaultRotateType(RotateType.ANGLE_270)
                .create()
        }
    }
