...
import com.lightresearch.probecamera.ausbc.callback.IButtonCallback;
import com.lightresearch.probecamera.ausbc.callback.IFrameCallback;
import com.lightresearch.probecamera.ausbc.callback.IStatusCallback;
...
public class UVCCamera {
	...
	protected int mCurrentWidth = 720, mCurrentHeight = 1280;
    ...
    public UVCCamera() {
		...
		Log.w(TAG, "mNativePtr: " + Long.toString(mNativePtr));
    	mSupportedSize = null;
	}
...
//    	if (DEBUG) XLogWrapper.v(TAG, "close:finished");
	...
	public Size getPreviewSize() {
		...
		for (final Size sz: list) {
			if ((sz.getWidth() == mCurrentWidth)
				|| (sz.getHeight() == mCurrentHeight)) {
				result =sz;
				break;
			}
		}
...
//================================================================================
	...
	public synchronized int getExposureMode(final int expsoreMode_abs) {
		int result = 0;
		if (mNativePtr != 0) {
			nativeUpdateExposureModeLimit(mNativePtr);
			final float range = Math.abs(mExposureModeMax - mExposureModeMin);
			if (range > 0) {
				result = (int)((expsoreMode_abs - mExposureModeMin) * 100.f / range);
			}
		}
		return result;
	}
	public synchronized int getExposureMode() {
		return getExposureMode(nativeGetExposureMode(mNativePtr));
	}
...
//================================================================================
	...
	public synchronized int getExposure(final int expsore_abs) {
		int result = 0;
		if (mNativePtr != 0) {
			nativeUpdateExposureLimit(mNativePtr);
			final float range = Math.abs(mExposureMax - mExposureMin);
			if (range > 0) {
				result = (int)((expsore_abs - mExposureMin) * 100.f / range);
			}
		}
		return result;
	}
