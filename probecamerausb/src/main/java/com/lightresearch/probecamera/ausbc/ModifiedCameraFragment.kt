...
import com.lightresearch.probecamera.ausbc.utils.CameraRequest
 ...
 */
abstract class ModifiedCameraFragment : BaseFragment(), ICameraStateCallBack {
    ...
    override fun initView() {
        Log.w(TAG, "INITING VIEW")
        when (val cameraView = getCameraView()) {
            is TextureView -> {
                Log.w(TAG, "IS TEXTURE VIEW")
                handleTextureView(cameraView)
                cameraView
            }
            is SurfaceView -> {
                Log.w(TAG, "IS SURFACE VIEW")
        ...
        mCameraClient = MultiCameraClient(requireContext(), object : IDeviceConnectCallBack {
            override fun onAttachDev(device: UsbDevice?) {
                ...
                context?.let {
                    ...
                    generateCamera(it, device).apply {
                        mCameraMap[device.deviceId] = this as MultiCameraClient.ICamera
                    }
            ...
            override fun onConnectDev(device: UsbDevice?, ctrlBlock: USBMonitor.UsbControlBlock?) {
                ...
                mCameraMap[device.deviceId]?.apply {
                    setUsbControlBlock(ctrlBlock as USBMonitor.UsbControlBlock)
                }?.also { camera ->
                    try {
                        mCurrentCamera?.cancel(true)
                        mCurrentCamera = null
    ...
    private fun handleTextureView(textureView: TextureView) {
        textureView.surfaceTextureListener = object : TextureView.SurfaceTextureListener {
            override fun onSurfaceTextureAvailable(p0: SurfaceTexture, width: Int, height: Int) {
                Log.w(TAG, "REGISTRING MULTI CAMERA")
                registerMultiCamera()
            }
     ...
     */
    protected open fun generateCamera(ctx: Context, device: UsbDevice): CameraUVC {
    ...
    protected fun openCamera(st: IAspectRatio? = null) {
        when (st) {
            ...
            getCurrentCamera()?.setCameraStateCallBack(this@ModifiedCameraFragment)
        }
    }
