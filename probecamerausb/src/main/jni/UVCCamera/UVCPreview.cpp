...
void *UVCPreview::capture_thread_func(void *vptr_args) {
	int result;
	__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "CAPTURE THREAD - VPTR ARGS: ")
	ENTER();
	__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "ENTERED: ");
	UVCPreview *preview = reinterpret_cast<UVCPreview *>(vptr_args);
	if (LIKELY(preview)) {
		JavaVM *vm = getVM();
		if (vm == nullptr) {
			__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "vm is NULL");
		} else {
			__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "vm is not NULL");
		}
		JNIEnv *env;
		__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "INITIALIZED ENV POINTER");
		// attach to JavaVM
		if (vm->GetEnv((void**)&env, JNI_VERSION_1_6) != JNI_OK) {
			jint res = vm->AttachCurrentThread(&env, NULL);
			if (res != JNI_OK) {
				__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera",
									"Failed to attach current thread");
			} else {
				__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera",
									"Successfully attached current thread");
			}
		}
		preview->do_capture(env);	// never return until finish previewing
		__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "FINISHED DO CAPTURE");
		// detach from JavaVM
		vm->DetachCurrentThread();
		MARK("DetachCurrentThread");
	} else {
		__android_log_write(ANDROID_LOG_ERROR, "com.lightresearch.probecamera", "Invalid UVCPreview* passed to capture_thread_func()");
	}
	PRE_EXIT();
	pthread_exit(NULL);
}
